...
window.PunchcardView = countlyView.extend({
  ...
  renderCommon: function(isRefresh) {
    var dataCSV = transformForD3(countlyPunchcard.getData());
    console.log(dataCSV);
...
$(document).ready(function() {
  ...
  var menu =
    ...
    "</a>";
  // append the menu item in mobile and web
...
function transformForD3(data) {
  ...
  // for each day of the week + label column
  for (var day = 0; day < 8; day++) {
    // for each hour of the day + label column
    for (var hour = 0; hour < stringArray[day].length; hour++) {
      // if hour is 0, it's label column
      if (hour === 0) {
        stringArray[day][hour] = daysOfWeek[day];
      }
      // if day is 0, it's label column
      if (day === 0) {
        stringArray[day][hour] = hour;
      }
      // loop the data
      for (var k = 0; k < data.length; k++) {
        // if we are in the current day/hour
        if (
  ...
  // starts with a ","
  var result = ",";
  // transform the matrix into a string for d3
  // stringArray.length = days
  for (var x = 0; x < stringArray.length; x++) {
    // stringArray[x].length = hours
    for (var y = 0; y < stringArray[x].length; y++) {
      // add the current value in the matrix
      result = result.concat(stringArray[x][y]);
      // if it's the last bucket in the row, append \n
